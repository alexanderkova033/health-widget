...
class AppContainer(context: Context) {
    ...
    // Serializes every push of the widget's UI to the home screen. Four independent triggers
    // can call this (the periodic tick worker, a style change, the manual "get a different
    // tip" button, and tapping the widget itself) with no ordering guarantee between them —
    // without a lock, two overlapping GlanceAppWidget.updateAll() calls could finish out of
    // order and leave a stale render on screen (the real, reported cause of the widget
    // background/tip only *sometimes* updating). Each call still re-reads the current
    // settings/tip from DataStore at execution time (nothing captures a stale snapshot), so
    // serializing just the push itself is enough: whichever call runs last always renders
    // whatever is actually persisted, regardless of which trigger queued first.
